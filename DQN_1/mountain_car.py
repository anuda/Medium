...
def get_car_location(screen_width):
    ...
    return int(env.state[0] * scale + (2 / 3) * screen_width)
...
def get_screen():
    ...
    screen = screen[:, int(screen_height*0.4):int(screen_height)]
    view_width = int(screen_width * 0.6)
    car_location = get_car_location(screen_width)
    # import ipdb;ipdb.set_trace()
...
plt.imshow(get_screen().cpu().squeeze(0).permute(1, 2, 0).numpy(),
           interpolation='none')
# import cv2
# cv2.imwrite('car.jpg',get_screen().cpu().squeeze(0).permute(1, 2, 0).numpy())
...
for i_episode in range(num_episodes):
    ...
    for t in range(1,10000):
        ...
        current_screen = get_screen()
        # xpos = get_car_location(env.render(mode='rgb_array').shape[1])
        xpos = env.state[0]
        if xpos> max_width:
            max_width = xpos
        if xpos<min_width:
